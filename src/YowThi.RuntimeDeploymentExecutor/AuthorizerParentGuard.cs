...
internal static class AuthorizerParentGuard
{
    ...
    {
        if (ExpectedAuthorizerExeSha256.All(ch => ch == '0'))
            throw new UnauthorizedAccessException("Runtime deployment executor authorizer identity is not provisioned.");
        ...
        if (!File.Exists(fullParentPath))
            throw new FileNotFoundException("Runtime deployment authorizer executable does not exist.", fullParentPath);
        ...
        if (!string.Equals(actualSha256, ExpectedAuthorizerExeSha256, StringComparison.OrdinalIgnoreCase))
            throw new UnauthorizedAccessException("Runtime deployment authorizer executable SHA-256 does not match the provisioned identity.");
    }
