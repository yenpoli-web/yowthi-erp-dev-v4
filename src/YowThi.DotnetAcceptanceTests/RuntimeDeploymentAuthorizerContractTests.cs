...
{
    [Fact]
    public void ApprovalSignatureGate_IsEcdsaP256AndFailClosedUntilP30Provisioning()
    {
    ...
    public void Authorizer_BindsP27RequestAndCanLaunchOnlyFixedP28Executor()
    {
        ...
        Assert.Contains("true,\n            now,", source, StringComparison.Ordinal);
    ...
    [Fact]
    public void ApprovalSchema_BindsRequestExecutorSignerNonceAndSignature()
    {
        ...
        Assert.Contains("\"signerKeyId\": { \"const\": \"p30-runtime-deployment-signer-v1\" }", schema, StringComparison.Ordinal);
    ...
    {
        var guard = File.ReadAllText(Path.GetFullPath(Path.Combine(
    ...
    private static string ReadAuthorizerSource(string fileName)
        => File.ReadAllText(Path.GetFullPath(Path.Combine(
            AppContext.BaseDirectory, "..", "..", "..", "..",
            "YowThi.RuntimeDeploymentAuthorizer", fileName)));
