...
internal static class InteractiveSessionGuard
{
    ...
    internal static void ValidateAtModuleLoad()
    {
        if (!OperatingSystem.IsWindows() || !Environment.UserInteractive)
            throw new UnauthorizedAccessException("Runtime deployment approval requires an interactive Windows user session.");
        ...
        if (!ProcessIdToSessionId((uint)Environment.ProcessId, out var currentSession) || currentSession != activeSession)
            throw new UnauthorizedAccessException("Runtime deployment approval bridge must execute in the active console user session.");
        ...
        if (!ProcessIdToSessionId((uint)parentId, out var parentSession) || parentSession != activeSession)
            throw new UnauthorizedAccessException("Runtime deployment approval bridge parent is not in the active console session.");
        ...
        if (!string.Equals(Path.GetFullPath(parentPath), expectedExplorer, StringComparison.OrdinalIgnoreCase))
            throw new UnauthorizedAccessException("Runtime deployment approval bridge may only be launched directly by Windows Explorer in the active console session.");
    }
    ...
    private static int GetParentProcessId()
    {
        ...
        if (status != 0 || value <= 0 || value > int.MaxValue)
            throw new UnauthorizedAccessException("Unable to establish runtime deployment approval bridge parent process identity.");
        return (int)value;
    }
